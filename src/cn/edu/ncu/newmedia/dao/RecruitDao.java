...
public class RecruitDao {
     ...
     * 一轮获取每个学生的信息
     * 想了半天，一轮二轮还是分开来写比较好
     * 经过启发，这种获取是不需要分开写的，但是最后的判定条件要多写一点，即where中的判定
     * 前面的逻辑即是默认第一个ID为0，选择出符合条件的符合条件的比当前ID大的第一个同学，并将选出来的id置为当前id
     */
    public Recruit get_one(int uid,long ucommunityId) throws Exception {
        ...
        String sql = "select * from recruit where community_id="+ucommunityId+" and id>+"+uid+" and message_state=1 order by id limit 1";
        ...
        if(rs.next()){
            ...
            recruit = new Recruit(id,studentId,communityId,name,sex,nation,birthday,nativePlace,politicalAffiliation,college,major,
    ...
    public void passone(String studentId,int communityId) throws Exception {
        ...
        Recruit recruit = null;
        conn = JdbcUtil.getConnection();
        String sql = "update recruit set state=2,message_state=1 where student_id="+studentId+" and community_id="+communityId;
        ps = conn.prepareStatement(sql);
     ...
     * 返回该社团总的报名人数
     */
    public int gettotal (long communityId) throws Exception {
    	Connection conn = null;
        PreparedStatement ps = null;
        Recruit recruit = null;
        conn = JdbcUtil.getConnection();
     ...
     */
    public Recruit get_two(int uid,int ucommunityId) throws Exception {
    	 Connection conn = null;
         PreparedStatement ps = null;
         Recruit recruit = null;
         conn = JdbcUtil.getConnection();
         String sql = "select * from recruit where community_id="+ucommunityId+" and state=2 and id> "+uid+" order by id limit 1";
         ps = conn.prepareStatement(sql);
         ResultSet rs = ps.executeQuery();
         if(rs.next()){
             int id = rs.getInt(1);
    ...
    public void passtwo(String studentId,int communityId) throws Exception {
        ...
        conn = JdbcUtil.getConnection();
        //若通过审核，将发送短信状态置为1
        String sql_message = "update recruit set message_state=1,state=3 where studentId="+studentId+" and community_id="+communityId;
